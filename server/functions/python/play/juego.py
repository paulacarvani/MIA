...
path = '/home/holbertonpc/Documents/Holbie/mia/server/functions/python/Imagenes'
images = []
clases = []
listades = os.listdir(path)
lista = sorted(listades, key=lambda x: os.path.getctime(os.path.join(path, x)), reverse=False)
# print(lista)
...
while True:
    ...
    if jug == 1:
        ...
        cv2.putText(frame, 'JUGUEMOS!', (250, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.71,(0, 255, 0), 2)

        # Mensaje inicio
        cv2.rectangle(frame, (145, 425), (465, 460), (0, 0, 0), -1)
        cv2.putText(frame, 'PRESIONA P PARA JUGAR', (150, 450), cv2.FONT_HERSHEY_SIMPLEX, 0.71, (0, 255, 0), 2)

        # Presiona P
        if t == 80 or t == 112 or fs == True:
            # Cambiamos bandera 'p'
            fs = True
